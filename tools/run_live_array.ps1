...
& $adb -s 001593529000048 reverse --remove-all
& $adb -s Y5GI8XVGPVRO99W8 reverse --remove-all
& $adb -s 001593529000048 reverse tcp:8765 tcp:8765
& $adb -s Y5GI8XVGPVRO99W8 reverse tcp:8765 tcp:8765

# 2. Reset applications
Write-Host "[2/3] Resetting dev.shruti on Phone 0 (Nothing) & Phone 1 (realme)..." -ForegroundColor Yellow
& $adb -s 001593529000048 shell am force-stop dev.shruti
& $adb -s Y5GI8XVGPVRO99W8 shell am force-stop dev.shruti

# 3. Start Live Array Processor (binds server first, then automatically signals phones to connect)
Write-Host "[3/3] Starting Live Array Processor & Web Dashboard ($Seconds seconds)..." -ForegroundColor Green
